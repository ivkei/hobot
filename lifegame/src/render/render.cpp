...
void Render(const hobot::Renderer& renderer, const Field& field){
  ...
  for (int i = 0; i < field.Width(); i++){
    for (int j = 0; j < field.Height(); j++){
      hobot::Vec2 fullDimensions = {2.0f/(float)field.Width(), 2.0f/(float)field.Height()};
      hobot::Vec2 dimensions = {fullDimensions.x*field.GetCellSize().x, fullDimensions.y*field.GetCellSize().y};

      hobot::Vec2 pos = {2.0f*((float)i/field.Width())-1.0f, 2.0f*((float)j/field.Height())-1.0f};
